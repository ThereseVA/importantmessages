    ...
    try {
      ...
      if (method === 'sharepoint') {
        ...
        spMessage = {
          ...
          Priority: 'Medium',
          Author: { Title: props.context?.pageContext?.user?.displayName || 'System' },
      ...
      if (spMessage) {
        ...
        result.message = `Message created (ID: ${messageId}) and distributed to ${result.success} channels`;
      } else {
      ...
      if (result.total === 0) {
        setResult('❌ No valid webhook URLs found');
      } else if (result.success === result.total) {
        setResult(`✅ Message sent successfully to ${result.total} channel${result.total > 1 ? 's' : ''}!`);
        // Clear form on success
        setTitle('');
        setMessage('');
      } else if (result.success > 0) {
        setResult(`⚠️ Partial success: ${result.success}/${result.total} channels succeeded`);
      } else {
        setResult(`❌ Failed to send to all ${result.total} channels`);
      }
